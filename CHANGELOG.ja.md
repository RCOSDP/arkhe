...
- 管理画面から台帳を組めるようにした（これまでは採番だけ）。**一覧にあった
  「NAAN を登録」「機関をオンボード」「shoulder を切り出す」「操作」の 4 つは、
  いずれも存在しないルートを指していて 404 だった。** 併せて NAAN と機関の
  設定画面を足した。
- `arkhe manager list` と `arkhe manager commitment`、`arkhe onboard --commitment`。
  約束の水準は `?` と `??` で公開されるのに**設定する手段が無く**、全機関が既定の
  `permanent-dynamic` を名乗ったままだった。機関が述べていない約束を機関の名前で
...
- `set_quota`。採番上限をオンボード時にしか決められなかったのを、後からも変えられる
  ようにした。**自機関では変えられない**——課された側が外せる上限は上限ではない。
- 一覧の画面名を「委譲の構造」から **機関管理** に変え、説明を台帳を初めて見る人に
  ...
  した（「機関をオンボード」→「機関を追加」、「shoulder を切り出す」→
  「名前空間を追加」）。機関を追加する画面には、**同時に名前空間を渡す**ことを
  最初に書いた。
...
- NAAN の設定画面を機関管理者が開けてしまい、保存で 403 になっていた。編集できる
  ように見えて保存できないフォームは、ボタンが出ないのに URL は通るのと同じ欠陥
  なので、開ける条件を保存できる条件に揃えた。
- NAA ポリシーを機関管理者が書き換えられた。これは名前空間を配る側の宣言で
  **NAAN 配下の全機関にかかる**ので、1 機関が他機関の分まで言い直せてはならない。
  NAAN 単位以上を要求するようにした。機関が自分について述べるのは約束の水準の
  ほうで、そちらは機関管理者が変えられる——この分かれ方は権限表ではなく、
  ARK の委譲の構造がそのまま出たもの。
