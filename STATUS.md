...
3. **継続的アーカイブ（PITR）を実際に組む**。手順は[デプロイ](docs/guides/deployment.md)に
   書いたが、組んで戻したことは無い——**日次のダンプだけでは、最大 24 時間ぶんの
   識別子が消える**。ほかの系なら取り込み直せばよいが、**この体系では採り直した
   時点で、外から見て `NR` 違反と区別がつかない**。
...
**まだ無いのは PITR である。** 日次のダンプだけでは最大 24 時間ぶんの識別子が
消え、**消えた ARK を採り直すのは外から見て `NR` 違反と区別がつかない**。
手順は文書に書いたが、組んで戻したことは無い。
