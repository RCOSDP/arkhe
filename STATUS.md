...
1. **`shoulder.redirect` の CLI**（`arkhe shoulder redirect <id> <template>`）。
   小さく、分散構成の自動化を今すぐ楽にする。画面と同じ `admin_ops` を呼ぶだけ。
2. **外部採番の取り込み口**。分散構成の穴の本体。入れるなら、名前が委譲した shoulder の
   内側にあること・二重採番でないこと・検査桁が合っていることを取り込み時に検査する
   ——**採番が更新に化けない**という不変条件を、取り込み経路でも守る必要がある。
3. **復元リハーサルと性能測定**。0.1 を名乗る前にやるべきこと。
4. **委譲先の外形監視**。運用の道具であって arkhe の機能とは限らない。
