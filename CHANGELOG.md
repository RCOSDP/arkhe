...
- **`scripts/ci.sh` and `scripts/release.sh`, to run CI and a release locally.** They
  mirror the workflows, down to **starting a disposable PostgreSQL and round-tripping the
  migrations** (never touching the demo database) — finding out after the push only costs
  a round trip. `release.sh` also checks the procedure itself: the tag matches the
  version, the CHANGELOG carries that section and its link definition in both languages,
  and the "Unreleased" comparison link points at the new version. Each of those has been
  got wrong before. **It does not push** — what has been released cannot be withdrawn, so
  the last step stays with a person.

### Fixed

- **The committed OpenAPI specs had fallen behind the implementation**
  (`docs/assets/openapi-*.json` still said `version: 0.0.1`). The published spec is
  regenerated by CI on every build, so nothing shipped wrong, but the repository
  disagreed with itself. `scripts/ci.sh` **caught this on its first run**, and now warns
