...
| `manager` | 1 組織ぶん。1 つの shoulder に固定することもできる |

**配られた側が、配った側より広く届くことはない。** NAAN 管理者はシステム管理者を
作れないし、組織管理者は他組織の主体を作れない。
...
- **承継** — 組織が別の組織に統合される。shoulder が移り、**ARK は何も変わらない**。
  変わるのは「今後誰が採番するか」だけ。
- **離脱** — 組織は存続するが、この基盤を離れる。**新規採番は止め、解決は永久に続ける。**
  転送先を組織自身のリゾルバへ向け直せば、以後こちらに作業を求めずに済む。

どちらも[承継と離脱](../guides/succession.md)で扱う。

## 委譲の先が、別の arkhe のとき

ここまでの委譲先は組織だったが、**同じ仕組みで別の arkhe に渡せる**。shoulder ごとに
下位の arkhe を置く構成、拠点ごとに NAAN を分ける構成、閉域に置いた arkhe を上位が
名前空間だけ把握する構成——それぞれの成り立ちと代償は
[分散して運用する](../guides/federation.md)にある。
