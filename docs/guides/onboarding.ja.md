...
NAAN を持っていない状態から、機関が採番できる状態までの通し。**arkhe の外で
...
⑤ 機関を迎え、名前空間を委譲する             arkhe onboard
⑥ 機関の約束の水準を確かめる                 arkhe manager commitment
...
ARK Alliance から**無償で**交付される。登録機関に払う金も、維持する会員資格も無い。
...
ある——ARK には代わりに永続性を保証してくれる登録機関がいないので、[約束はこちらの
...
**機関の登録と shoulder の委譲は必ず対で起きる。** 分けられない——名前空間を持たない
機関は採番できないので、台帳に置く意味がない。
...
**管理画面からも変えられる**（一覧の機関行の「操作」）。ここは
[機関管理者自身も変えられる](admin.md#設定を変える)——約束は機関自身のもので、
述べる主体が述べられなければ意味がない。

`--commitment` を付けずに迎えると既定の `permanent-dynamic` が付く。**既定のまま
置いてはいけない。** この値は `?` と `??` でそのまま公開されるので、放置すると
**機関が述べていない約束を、機関の名前で名乗る**ことになる。宣言していないものを
...
| | 名乗り方 | 持てる資格情報 |
| --- | --- | --- |
| 人（機関管理者など） | 外部ログイン（OIDC / proxy）またはパスワード | API キー・client_secret は**持てない** |
| 機械（リポジトリ本体など） | API キー / client_secret | パスワードは**持てない** |

```bash
# 機関管理者。client_id は認可サーバが返す識別子（メール・eppn など）
...
(3) 用途ごとに scope を絞れない。`--shoulder` で固定しておけば、鍵が漏れても他機関の
名前空間には届かない。
...
機関が統合されたり去ったりしたときは[承継と離脱](succession.md)を使う。どちらも
...
| ④ が ⑤ より先 | 方針を述べていない NAAN の下に機関ができる |
| ⑤ は不可分 | 採番できない機関が台帳に残る |
| ⑥ は ⑤ の直後 | 既定値が機関の宣言として公開されたまま動き出す |
