...
Institutions merge, split and leave. **The identifiers must not notice.**

`ark:/99999/x9abc` was handed out. Under NR it cannot be reissued, so it cannot be
replaced by a new name at a new institution — that would kill the original. What can
change is who mints next, and where the target points.

## Succession — an institution merges into another

```bash
arkhe succeed 1 2          # institution 1 is succeeded by institution 2
```
...
## Departure — an institution leaves but still exists
...
institution's own resolver **and** sets the same delegation on the shoulder, so that
even names this ledger never saw are forwarded there. After that, **nothing further is
required of the departed institution**.
