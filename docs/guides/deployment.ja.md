...
| `shared_buffers` | **索引が載れば足りる** | 既定 128 MB で**キャッシュ命中 99.95%**。上げても変わらなかった（下の「振ってみて、効かなかったもの」）——一般の目安「メモリの 25%」は、この負荷では買えるものが無い |
...
**効かないと分かっていることのほうが、運用では役に立つ。** 同じ台で片端から試した:

| 試したこと | 結果 |
| --- | --- |
| `uvloop` / `httptools` に替える | **差なし**（1,280 → 1,400 → 1,275 rps。ばらつきの内） |
| `shared_buffers` 128MB / 512MB / 2GB | **差なし。** どれも**キャッシュ命中 99.95%** |
| 接続プールを 30 倍に振る | **差なし**（上の ③） |
| `synchronous_commit=off`（採番） | **差なし**（51.3 → 51.7 rps） |

**`shared_buffers` が効かない理由**——解決は**索引 1 回と行 1 本**しか触らないので、
作業集合が小さい。残りは OS のページキャッシュに載るので、**PostgreSQL 側に
大きく取っても行き先が変わらない**。「メモリの 25%」は一般の目安として正しいが、
**この負荷では買えるものが無い**。要るのは**索引が載ること**（100 万件で 39 MB）だけで、
既定の 128MB でも足りている。

**`synchronous_commit` が効かない理由**——採番の 1 件は **p50 で約 60 ms、うち
53 ms が Argon2** である。**ディスクへの同期は測れるところに出てこない**。
**安全を削っても速くならないなら、削る理由が無い。**

#### ⑥ 唯一効いたもの——`pre_ping`

**接続を貸す前の生存確認（`SELECT 1`）が、解決 1 件あたりの DB 往復をほぼ倍にしていた。**

| | 要求 500 件あたりの往復（`xact_commit + xact_rollback`） |
| --- | --- |
| `ARKHE_DB_PRE_PING=true`（既定） | **802 回**（1 件あたり 1.60） |
| `ARKHE_DB_PRE_PING=false` | **411 回**（1 件あたり 0.82） |

**rps では見えなかった**（ばらつきに埋もれる）。**数えたら決定的だった**
——解決はもともと索引 1 回しか引かないので、**確認の 1 往復が相対的に重い**。

**既定は入れたままにしてある。** 切ってよいのは、**DB が近く、切られた接続が
そのまま例外として呼び出し側に出ても構わない**構成である。前段が接続を黙って切る
なら、切る前に `ARKHE_DB_POOL_RECYCLE` を設定すること。

**この節でいちばん言いたいこと。** 振れる摘みはたくさんあるが、**測って効いたのは
これ 1 つだった**。ほかは**既定のままでよい**——**効かない調整に時間を使うより、
worker を 4 にして、それでも足りなければ DB を横に増やすほうが早い。**

#### ⑦ DB を横に増やすと、何が起きるか

**レプリカを実際に組んで測った**（ストリーミング複製、resolver を
`ARKHE_READ_DATABASE_URL` でレプリカへ）。

| | rps | 負荷中の主系 CPU | 負荷中のレプリカ CPU |
| --- | --- | --- | --- |
| 読みも主系 | 1,433 | **136%** | 0% |
| 読みはレプリカ | 1,307 | **0%** | 135% |

**rps は増えない。** 同じ 1 台の上なので当然で——**レプリカは CPU を増やさない**。
台を分けて初めて増える。

**増えるのは「主系の余裕」である。** 読みの負荷が**主系から完全に外れる**
（136% → 0%）。つまりレプリカが買うのは速さではなく、**読みが荒れても採番と管理が
巻き込まれないこと**である。解決は止められないが採番は止めてよい、という非対称の
裏返しで、**止めてよいほうを守っている。**

**主系が落ちても解決は続く**（実際に止めて確かめた——`302`、`?info` も `200`、
`/readyz` も `200`）。レプリカは読める待機系なので、**採番だけが止まる。**

#### 複製の遅れは、どのくらいか

**採番してから、レプリカで解決できるようになるまでを測った:**

| | 中央値 | 最大 | 1 回目の問い合わせで解決できた |
| --- | --- | --- | --- |
| 静かなとき（30 回） | **4.8 ms** | 74 ms | **30 / 30** |
| 書き込み負荷の最中（20 回） | **3.4 ms** | 6.3 ms | **20 / 20** |

`pg_stat_replication` の `replay_lag` は負荷中でも **1.3 ミリ秒**だった。

**つまり、同じ場所に置いた健全な構成では、この危険は現れない。** 上の
[監視](#レプリケーション遅延はこの体系では正しさの問題である)で「採番直後に引かれると
`404`」と書いたが、**測った範囲では 1 度も起きなかった**。

**それでも監視する理由は残る。** 遅れが伸びるのは、**別の機械に置いて往復が乗ったとき、
レプリカが読み負荷で詰まったとき、大量投入で再生が追いつかないとき**である
——いずれもここでは再現していない。**起きないことを確かめたのではなく、
この条件では起きなかった**、というだけである。
