...
**併用できる。** `ARKHE_AUTH=apikey,oidc` は移行期に普通に要る形で、機関ごとに好きな
時期に移れる（一斉切り替えを強いない）。
...
結びつける**こと。arklet は NAAN 単位でしか認可できず、**同一 NAAN 内で他機関の
名前空間に採番できた**。
...
認可サーバを運用できない機関のための入口。**grant は client_credentials だけ**——
...
**到達範囲は毎回台帳から引き直す**——鍵の失効や機関の統廃合が次のリクエストから効く。
