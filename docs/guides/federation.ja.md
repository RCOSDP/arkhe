...
- **転送を一時的に止める手段。** 委譲先のリゾルバが落ちた・乗っ取られた・間違った
  行き先を配ってしまった——**どれも急いで止めたいが、識別子は殺したくない**。
  ARK 単位なら `url` を空にすれば記述が返る（D6）が、**元の行き先も理由も期限も
  残らない**ので、戻すのは人の記憶に頼ることになる。shoulder / NAAN 単位には
  手段が無い（`redirect` を書き換えると元の値が失われる）。設計案は
  [STATUS.md](https://github.com/RCOSDP/arkhe/blob/main/STATUS.md) にある。
