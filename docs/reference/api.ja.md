...
**公開前に採ることができ、そのあいだは消せる。** `POST /api/mint` に
`{"reserve": true}` を付けると、その ARK は**公開のリゾルバでは解決しない**（知らない
名前と同じに答える。閉域のリゾルバは `ARKHE_RESOLVE_UNPUBLISHED` で解決する）。`POST /api/publish` で公開すると解決を始め、**以後は削除できない**
——`POST /api/delete` は `409` で断る。取り下げた名前は二度と採られない。
[壊さないもの](../concepts/invariants.md#before-publication)を参照。

**公開は取り下げられ、出し直せる。** `POST /api/unpublish`（`ark:unpublish`）は行を
残したまま解決を止め、`POST /api/publish` で戻す。どちらも**呼ぶ側の届く範囲の
内側**だけ——組織は自分の shoulder、NAAN 管理者は自 NAAN。**一度でも外に出した名前**
なので、取り下げには `reason` と、ARK を打ち直す `confirm` が要る。削除も同じで、
一度でも公開したものには同じ 2 つを要求する。**重さは名前の履歴に従い、呼ぶ側の位
には従わない。**

**捨てる一括は 1 リクエストで済む。** `POST /api/delete/bulk`（`ark:delete`）は
**一度も公開していない** ARK をまとめて消す——予約でまとめて採るのが実際の使い方である
以上、**捨てるのも採るのと同じ安さでなければ**、放置された番号が台帳に残るだけになる。
**公開中、または一度でも公開した行が 1 つでもあれば、リクエスト全体が `409` で落ちる**。
その名前は `POST /api/delete` を 1 件で通す——理由と ARK の打ち直しを求める経路である。

**`POST /api/purge` は 2 手を一度に行う**（`ark:purge`、届く範囲の内側）。裁判所の
削除命令や、公開してはならなかった個人情報のためのもので、**逃げ道が無ければ誰かが
DB を直接叩く**からである。理由が要り、`confirm` に ARK を打ち直させ、監査に残る。
**名前は解放しない**ので、残っている参照は `404` になるだけで、別のものを指すことは
ない。
