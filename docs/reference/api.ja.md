...
**公開したものを消す道は、1 本だけある。** `POST /api/purge` は **RA の運用者だけ**
（`ark:purge` かつ `authority=system`）が使える。裁判所の削除命令や、公開しては
ならなかった個人情報のためのもので、**逃げ道が無ければ誰かが DB を直接叩く**からで
ある。理由が要り、`confirm` に ARK を打ち直させ、監査に残る。**名前は解放しない**
ので、残っている参照は `404` になるだけで、別のものを指すことはない。
