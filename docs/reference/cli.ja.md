...
| `arkhe ark delete` | **公開していない ARK を消す。** 公開中のものは先に取り下げる。**一度でも公開した名前なら理由が必須で、確認を訊く。** 複数並べると一括の経路を通り、**一度でも公開した名前が混じっていれば 1 件も消さない**。`-` を渡すと標準入力から 1 行 1 件で読む。消えるのは行だけで、**その名前は二度と採られない。** |
...
# 放置された予約をまとめて捨てる。訊かれるのは 1 回。**先に一覧を見ること**（戻せない）
...
arkhe ark list --state reserved --older-than 365 --limit 1000 | awk '{print $1}' | \
  arkhe ark delete - --reason "審査が取りやめになった"
```
