...
def test_機関管理者は自機関の約束を変えられる(db, world, principal_of, as_principal):
    """**約束は機関自身のもの。** 述べる主体が述べられなければ意味がない。"""
    a = world["a"]
...
def test_機関管理者は他機関の約束を変えられない(db, world, principal_of, as_principal):
    a, b = world["a"], world["b"]
    c = as_principal(principal_of(manager=a))
...
def test_システム管理者はどの機関の約束も変えられる(db, world, principal_of, as_principal):
...
def test_機関管理者は自機関の採番上限を外せない(db, world, principal_of, as_principal):
...
def test_NAAポリシーは機関管理者には変えられない(db, world, principal_of, as_principal):
    """**NAAN 配下の全機関にかかる宣言。** 1 機関が他機関の分まで書き換えられない。"""
...
def test_自機関の設定画面は開ける(world, principal_of, as_principal):
    c = as_principal(principal_of(manager=world["a"]))
...
def test_一覧に自機関への導線が出る(world, principal_of, as_principal):
    """**約束は機関自身のものなので、機関管理者にも導線を出す。**"""
    a = world["a"]
...
def test_オンボードは画面からもできる(db, world, principal_of, as_principal):
    ...
    r = c.post("/admin/manager/new", data={
        "naan": "99999", "name": "D機関", "shoulder": "/d4",
        "commitment": "permanent-stable", "quota": "100",
    })
    assert r.status_code == 303
    m = db.scalar(db.query(Manager).filter_by(name="D機関").statement)
